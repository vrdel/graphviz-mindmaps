...
For justfile projects generated by `create-mm`, `target-make mindmap-01.otl` runs `just -f justfile build mindmap-01.otl`; `target-make montage.yml` runs `just -f justfile build montage.yml`. Add the optional `p` argument to invoke the `buildpreview` recipe instead, for example `just -f justfile buildpreview mindmap-01.otl`.
