...
When a target is found inside `montage.yml`, `target-make` runs the montage recipe for that YAML file. For montage projects generated by `create-mm -m`, that means `just montage`.
